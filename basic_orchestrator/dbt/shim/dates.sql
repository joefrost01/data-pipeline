...
{#
    End of time sentinel for bi-temporal tables.
    
    Returns 9999-12-31 23:59:59 as timestamp.
#}
...
{#
    ...
    BigQuery: PARSE_DATE('%Y%m%d', str)
    DuckDB:   strptime(str, '%Y%m%d')::date
#}

{% macro parse_date(format_string, string_field) %}
    {% if is_duckdb() %}
        strptime({{ string_field }}, '{{ format_string }}')::date
    {% else %}
        parse_date('{{ format_string }}', {{ string_field }})
    {% endif %}
{% endmacro %}
